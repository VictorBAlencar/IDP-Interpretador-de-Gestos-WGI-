...
def auto_calibrate(gesture_name, idx_raw_history, mid_raw_history, thumb_dist_history=None):
    ...
    idx_mean = statistics.mean(idx_raw_history)
    mid_mean = statistics.mean(mid_raw_history)
    
    BUFFER = 20 
    
    if gesture_name == "left_click":
        config[gesture_name]["idx_angle_max"] = idx_mean + BUFFER
        config[gesture_name]["mid_angle_min"] = max(0, mid_mean - BUFFER)
    elif gesture_name == "right_click":
        config[gesture_name]["idx_angle_min"] = max(0, idx_mean - BUFFER)
    ...
    if thumb_dist_history and len(thumb_dist_history) > 0:
        thumb_mean = statistics.mean(thumb_dist_history)
        config[gesture_name]["thumb_index_dist_min"] = max(0, thumb_mean - 10)
